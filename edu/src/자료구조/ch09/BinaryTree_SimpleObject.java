...
class Tree4 {
	...
	public boolean add(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		TreeNode4 tmp = new TreeNode4(obj), p = root, q = null;
		if(p == null) {
			root = tmp;
			return true;
		}
	...
	public boolean delete(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
		...
		//root delete 할 때
		if (c.compare(obj, p.data) == 0 && p.LeftChild == null && p.RightChild == null) {
			root = null;
			return true;
		}
		if (c.compare(obj, p.data) == 0 && p.LeftChild == null) {
			root = root.RightChild;
			return true;
		}
		if (c.compare(obj, p.data) == 0 && p.RightChild == null) {
			root = root.LeftChild;
			return true;
		}
		while(p != null) {
