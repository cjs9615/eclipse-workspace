...
class DoubledLinkedList2 {
    ...
    public boolean search(SimpleObject2 obj, Comparator<? super SimpleObject2> c) {
        Node4 tmp = first.rlink;
        while(tmp != first) {
            if(c.compare(tmp.data, obj) == 0) return true;
            tmp = tmp.rlink;
        }
        return false;
    }
    ...
    public DoubledLinkedList2 merge(DoubledLinkedList2 lst2) {
        DoubledLinkedList2 result = new DoubledLinkedList2();
        if(lst2.isEmpty()) return this;
        Node4 p = this.first.rlink;
        while(p != first) {
            result.add(p.data, SimpleObject2.NO_ORDER);
            p = p.rlink;
        }
        p = lst2.first.rlink;
        while(p != lst2.first) {
